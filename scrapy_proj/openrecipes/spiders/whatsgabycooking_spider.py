...
class WhatsgabycookingMixin(object):
    ...
    def parse_item(self, response):
        ...
        image_path = hxs.select(
            "descendant-or-self::img[@class and contains(@class, 'wp-image')][1]/@data-lazy-src"
        ).extract()
...
class WhatsgabycookingcrawlSpider(CrawlSpider, WhatsgabycookingMixin):
    ...
    rules = (
        Rule(LinkExtractor(allow=("/category/categories/.*/"))),
        Rule(
            LinkExtractor(allow=("/[^/]+/"), deny=("/category/categories/.*/")),
            callback="parse_item",
        ),
    )
