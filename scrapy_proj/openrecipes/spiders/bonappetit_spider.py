...
class BonappetitMixin(object):
    ...
    def parse_item(self, response):
        ...
        recipeYield_path = (
            '//div[@class="time-and-yield"]/*/span[@class="yield"]/text()'
        )
...
class BonappetitcrawlSpider(CrawlSpider, BonappetitMixin):
    ...
    rules = (
        Rule(LinkExtractor(allow=("services/sitemap/recipes?page=\d+"))),
        Rule(LinkExtractor(allow=("recipes/\d{4}/\d{2}/.+")), callback="parse_item"),
    )
