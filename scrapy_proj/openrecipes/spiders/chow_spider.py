    ...
    def parse_item(self, response):
        ...
        for r_scope in recipes_scopes:
            ...
            for i_scope in ingredient_scopes:
                ...
                ingredients.append(
                    " ".join(i.strip() for i in ingredient).encode("utf-8")
                )
...
class ChowSpider(CrawlSpider, ChowMixin):
    ...
    start_urls = ["http://www.chow.com/recipes?page=2"]
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=("recipes\/\d\d\d\d\d-.+$")), callback="parse_item"),
    )
