...
class DelishhhcrawlSpider(CrawlSpider, DelishhhMixin):
    ...
    rules = (
        ...
        Rule(
            LinkExtractor(allow=("\d\d\d\d\/\d\d\/\d\d\/(.+)\/?")),
            callback="parse_item",
        ),
    )
