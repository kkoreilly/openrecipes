...
class ElanaspantryMixin(object):
    ...
    def parse_item(self, response):
        ...
        for r_scope in recipes_scopes:
            ...
            il.add_value(
                "recipeYield", " ".join(r_scope.select(recipeYield_path).extract())
            )
            ...
            for ingredient_node in ingredient_scopes:
                ingredient = [
                    i.strip()
                    for i in ingredient_node.select(
            ...
            il.add_value(
                "datePublished",
                datePublished.replace("Posted on", "").replace("in", "").strip(),
            )
...
class ElanaspantrycrawlSpider(CrawlSpider, ElanaspantryMixin):
    ...
    rules = (
        Rule(LinkExtractor(allow=("/gluten-free-recipes/.+/"))),
